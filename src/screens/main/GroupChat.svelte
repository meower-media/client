...
<script>
	import {
		auth_header, user,
		chatName, chatMembers, chatid,
		spinner, mainPage as page,
		modalShown, modalPage,
		profileClicked_GC, lastTyped
	} from "../../lib/stores.js";

	import Member from "./groupchat/Member.svelte";
	...
	if (clm.link.ws.readyState === 1) {
		...
		clm.link.ws.addEventListener("open", listenOnLink);
	}
</script>
			...
			{#if $user.name}
				<form
					...
					on:submit|preventDefault={e => {					
						...
						const postListener = clm.link.on("statuscode", cmd => {
							if (cmd.listener !== "post_chat") return;
							clm.link.off(postListener);
							spinner.set(false);
				...
				>
					<textarea
						...
						use:autoresize
						on:input={() => {
							if ($lastTyped + 1500 < Date.now()) {
								lastTyped.set(Date.now());
								clm.link.send({
									cmd: "direct",
									val: {
										cmd: "set_chat_state",
