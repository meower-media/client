...
<div class="Search">
    ...
    <Container>
		...
		{:then resp}
			...
			{:then stats}
				There are {(stats.posts / 1000).toFixed(2)}k posts, {stats.chats} chats and {(stats.users / 1000).toFixed(2)}k users on Meower.
			{/await}
		{/await}
    </Container>
    <Container>
        ...
        <form
            ...
            on:submit|preventDefault={async (e) => {
				...
				try {
					...
					const url = apiUrl + "users/" + encodeURIComponent(username);
        ...
        >
			<p>Note that usernames are currently case-sensitive!</p>
            <input
                type="text"
                class="white"
