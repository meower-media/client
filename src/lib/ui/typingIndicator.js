...
import {link} from "../networking/clmanager.js";
...
link.on("direct", cmd => {
    ...
    if (cmd.val.type === 1) {
        ...
        _typing[cmd.val.post_origin] = _typing[cmd.val.post_origin].filter(selectedUser => selectedUser !== cmd.val.u);
        typing.set(_typing);
    }
});
...
function checkIfStillTyping(chat, user, typingStart) {
    if (typingSince[chat][user] === typingStart) {
        _typing[chat] = _typing[chat].filter(selectedUser => selectedUser !== user);
        typing.set(_typing);
    }
}
