<script>
    ...
    $: {
        Object.entries(stores).forEach(([key, value]) => {
            value.subscribe(v => parsedStores[key] = [typeof(v), JSON.stringify(v)]);
        });
    }
</script>
                ...
                <div class="settings-controls">
                    {#if editingStore === storeName}
                        ...
                        <button
                            ...
                            on:click={() => {
                                try {
                                    stores[storeName].set(storeType === "object" ? JSON.parse(editingStoreVal) : editingStoreVal);
                                    editingStore = "";
                                } catch (e) {
                                    saveStoreError = e;
                                }
                    ...
                    {:else}
                        <button
                            ...
                            on:click={() => {
                                saveStoreError = "";
                                editingStoreVal = storeType === "object" ? storeVal : JSON.parse(storeVal);
